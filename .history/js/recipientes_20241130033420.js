function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        const densidadLiquido = parseFloat(document.getElementById("Densidad").value);
        const pesoMetal = parseFloat(document.getElementById("peso").value);

        if (isNaN(diametro) || isNaN(altura) || isNaN(densidadLiquido) || isNaN(pesoMetal)) {
            window.alert("Por favor, ingrese valores válidos.");
            return;
        }
        ...
        //peso del liquido
        const pesoLiquido = densidadLiquido * volumen;

        //peso total
        const pesoTotal = pesoLiquido + pesoMetal;

        //mostrar
        document.querySelector("#resultadoVolumen").value = volumen.toFixed(2);
        document.querySelector("#resultadoPesoLiquido").value = pesoLiquido.toFixed(2);
        document.querySelector("#resultadoPesoTotal").value = pesoTotal.toFixed(2);
